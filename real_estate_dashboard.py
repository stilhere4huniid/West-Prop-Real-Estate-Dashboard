...
if 'is_admin' not in st.session_state:
    st.session_state.is_admin = False  # Default to non-admin access

# Initialize email related session states
...
if page == "🏠 Simulator":
    ...
    st.sidebar.write("Debug - ROI Values:")
    st.sidebar.write(f"ROI: {roi}")
    st.sidebar.write(f"Smart ROI: {smart_roi}")
    st.sidebar.write(f"Predicted ROI: {predicted_roi}")
...
if page == "🗺️ ROI Map":
    ...
    # Create map centered on the first property if available, or default location
    if not filtered_df.empty:
        map_center = [filtered_df["Latitude"].iloc[0], filtered_df["Longitude"].iloc[0]]
    else:
        map_center = [-17.8, 31.02]  # Default Harare coordinates
        ...
        for _, row in filtered_df.iterrows():
            ...
            if pd.notna(row["Latitude"]) and pd.notna(row["Longitude"]):
                folium.Marker(
                    ...
                    popup=f"{row['Project']} — ROI: {row['ROI (%)']:.1f}%",
                    tooltip=row["Project"]
                ).add_to(m)
    ...
    map_data = st_folium(m, width=700, height=500, returned_objects=["last_clicked", "bounds", "zoom"])
    
    # Add email sharing feature
    st.markdown("---")
    st.markdown("### 📧 Share This Map View")
    
    with st.expander("Email This Map View", expanded=False):
        with st.form("map_email_form"):
            email = st.text_input("Recipient's Email")
            message = st.text_area("Your Message", 
                                value=f"Here's a custom property view from WestProp's ROI Map.\n\n"
                                    f"Current filters:\n"
                                    f"- Minimum ROI: {min_roi}%\n"
                                    f"- Show Heatmap: {'Yes' if show_heatmap else 'No'}\n"
                                    f"- Show Markers: {'Yes' if show_markers else 'No'}\n\n"
                                    f"Properties in this view: {len(filtered_df)}")
            
            if st.form_submit_button("Send Map View"):
                if not email or "@" not in email:
                    st.error("Please enter a valid email address")
                else:
                    with st.spinner("Sending your map view..."):
                        try:
                            from email_service import send_email
                            
                            # Prepare the email content
                            subject = f"WestProp ROI Map View: {len(filtered_df)} Properties (ROI ≥ {min_roi}%)"
                            
                            # Create a text summary of the properties
                            properties_summary = []
                            for _, row in filtered_df.head(10).iterrows():  # Limit to top 10 for email
                                # Safely format price with commas if it's a number, otherwise use as is
                                ...
                                prop_info = (
                                    ...
                                    f"📐 {row.get('Size (sqm)', 'N/A')} sqm\n"
                                )
                                properties_summary.append(prop_info)
                            
                            # Get map view and simulation details
                            center_lat = map_data.get('center', {}).get('lat', 'N/A')
                            center_lng = map_data.get('center', {}).get('lng', 'N/A')
                            zoom_level = map_data.get('zoom', 'N/A')
                            simulation_details = []
                            
                            # Initialize center_str with a default value
                            center_str = f"{center_lat}, {center_lng}"
                            
                            # Format coordinates if they're numbers
                            if isinstance(center_lat, (int, float)) and isinstance(center_lng, (int, float)):
                                center_str = f"{center_lat:.4f}, {center_lng:.4f}"
                            
                            # Check if there's a simulated ROI in session state
                            if 'simulation_data' in st.session_state and st.session_state.simulation_data.get('clicked', False):
                                sim_data = st.session_state.simulation_data
                                
                                if sim_data['is_developed']:
                                    simulation_details.extend([
                                        "\n🔍 ROI Simulation at Clicked Location:",
                                        f"- Nearest Project: {sim_data['project_name']}",
                                        f"- Distance from Project: ~{sim_data['distance_km']:.2f} km",
                                        f"- Base ROI: {sim_data['base_roi']:.2f}%",
                                        f"- Distance-Adjusted ROI: {sim_data['simulated_roi']:.2f}%"
                                    ])
                                else:
                                    simulation_details.extend([
                                        "\n🔍 ROI Simulation at Clicked Location:",
                                        "- Area: Undeveloped",
                                        f"- Estimated Base ROI: {sim_data['base_roi']:.2f}%",
                                        f"- Simulated ROI: {sim_data['simulated_roi']:.2f}%"
                                    ])
                            
                            # Build the map info section
                            map_info = [
                                "\n---\n",
                                "🔍 Map View Details:",
                                f"- Center: {center_str}",
                                f"- Zoom Level: {zoom_level}",
                                f"- Properties Shown: {len(filtered_df)}",
                                f"- Minimum ROI: {min_roi}%"
                            ]
                            
                            # Add simulation details if available
                            if simulation_details:
                                map_info.extend(simulation_details)
                            
                            # Add section header for properties
                            map_info.append("\n🏢 Top Properties in This View:\n")
                            ...
                            if len(filtered_df) > 10:
                                email_body += f"\n...and {len(filtered_df) - 10} more properties. View the full list in the dashboard.\n"
                            
                            email_body += "\n---\n"
                            email_body += "\nThis email was sent from WestProp's Interactive ROI Map.\n"
                            email_body += " WestProp Holdings. All rights reserved."
                            
                            # Send the email
                            if send_email(email, subject, email_body):
                                st.success("✅ Map view sent successfully!")
                                st.balloons()
                            else:
    ...
    if map_data and map_data.get("last_clicked"):
        ...
        nearest_project = df.sort_values('distance').iloc[0]
        
        # Calculate ROI based on distance to nearest project
        ...
        if distance < max_distance:
            ...
            st.markdown(f"📏 **Distance from project center:** ~{distance*111:.2f} km")
        else:
            # If far from any project, generate a random ROI based on area
            base_roi = 5.0  # Base ROI for undeveloped areas
            ...
            st.markdown("📍 **You clicked on an undeveloped area**")
    ...
    if st.session_state.get("voting_admin_logged_in", False):
        ...
        try:
            ...
            if recent_votes.empty:
                ...
                with tab2:
                    ...
                    if 'Project' in recent_votes.columns:
                        ...
                        for i, (idx, row) in enumerate(location_votes.iterrows()):
                            total = row['Total']
                            if 'Yes' in row:
                                ax2.text(i, row['Yes']/2, f"{row['Yes']} ({row['Yes']/total*100:.1f}%)", 
                                        ha='center', va='center', color='white', fontweight='bold')
                            if 'No' in row:
                                ax2.text(i, row['Yes'] + row.get('No', 0)/2, 
                                        f"{row.get('No', 0)} ({row.get('No', 0)/total*100:.1f}%)", 
                                        ha='center', va='center', color='white', fontweight='bold')
                        ...
                        ax3.pie(vote_counts, 
                               ...
                               startangle=90)
                        ax3.set_title('Overall Vote Distribution')
                        ...
                        st.markdown("#### Detailed Breakdown by Location")
                        st.dataframe(location_votes.style.background_gradient(cmap='YlGnBu', axis=0))
                    else:
                                    ...
                                    try:
                                        ...
                                        <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto;">
                                            ...
                                            <p style="color: #555; font-size: 14px; margin-bottom: 20px;">
                                                Here's a summary of recent voting activity as of {dt.now().strftime('%Y-%m-%d %H:%M')}:
                                            </p>
                                            
                                            <div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; margin-bottom: 20px;">
                                                <h3 style="color: #2c3e50; margin-top: 0;">📊 Overall Statistics</h3>
                                                <ul style="list-style-type: none; padding: 0; margin: 0;">
                                                    <li style="margin-bottom: 5px;"><strong>Total Votes:</strong> {total_votes:,}</li>
                                                    <li style="margin-bottom: 5px;"><span style="color: #27ae60; font-weight: bold;">✓ Yes Votes:</span> {yes_votes:,} ({yes_pct:.1f}%)</li>
                                                    <li><span style="color: #e74c3c; font-weight: bold;">✗ No Votes:</span> {no_votes:,} ({(100 - yes_pct):.1f}%)</li>
                                                </ul>
                                            </div>
                                            ...
                                            <div style="margin: 25px 0;">
                                                ...
                                                {recent_votes[['Timestamp', 'Project', 'Vote']].tail(10).to_html(index=False, 
                                                                                                          classes='dataframe',
                                                                                                          justify='center',
                                                                                                          border=0)}
                                            </div>
                                            ...
                                            <div style="margin-top: 30px; padding-top: 15px; border-top: 1px solid #eee; color: #7f8c8d; font-size: 12px;">
                                                <p>This is an automated report from WestProp's Voting System.<br>
                                                For any questions, please contact the system administrator.</p>
                                            </div>
                                        </div>
                                        """
                                        ...
                                        with tempfile.TemporaryDirectory() as temp_dir:
                                            ...
                                            try:
                                                ...
                                                ax1.set_xlabel('Vote')
                                                ax1.set_ylabel('Number of Votes')
                                                overall_img = os.path.join(temp_dir, 'overall.png')
                                                fig1.savefig(overall_img, bbox_inches='tight', dpi=100)
                                                plt.close(fig1)
                                                img_paths.append(overall_img)
                                                
                                                # If location data is available, save location-based visualizations
                                                if 'Project' in recent_votes.columns:
                                                    ...
                                                    for i, (idx, row) in enumerate(location_votes.iterrows()):
                                                        total = row['Total']
                                                        if 'Yes' in row and row['Yes'] > 0:
                                                            ax2.text(i, row['Yes']/2, f"{row['Yes']} ({row['Yes']/total*100:.1f}%)", 
                                                                    ha='center', va='center', color='white', fontweight='bold')
                                                        if 'No' in row and row.get('No', 0) > 0:
                                                            ax2.text(i, row.get('Yes', 0) + row.get('No', 0)/2, 
                                                                    f"{row.get('No', 0)} ({row.get('No', 0)/total*100:.1f}%)", 
                                                                    ha='center', va='center', color='white', fontweight='bold')
                                                    ...
                                                    location_img = os.path.join(temp_dir, 'by_location.png')
                                                    fig2.savefig(location_img, bbox_inches='tight', dpi=100)
                                                    plt.close(fig2)
                                                    img_paths.append(location_img)
                                                
                                                # Add visualizations to email body
                                                email_body += """
                                                <h3>📊 Voting Visualizations</h3>
                                                <div style='margin: 20px 0;'>
                                                    <h4>Overall Vote Distribution</h4>
                                                    <img src='cid:overall' style='max-width: 100%; border: 1px solid #ddd; border-radius: 4px;'/>
                                                """
                                                ...
                                                # Send the email with visualizations
                                                if send_email(
                                                    email, 
                                                    subject, 
                                                    email_body, 
                                                    ...
                                                    image_cids=['overall', 'bylocation'] if 'Project' in recent_votes else ['overall']
                                                ):
                                                    status_container.success("✅ Vote report with visualizations sent successfully!")
                                                    st.balloons()
                                                else:
                ...
                # Reset button
                if st.button("🧹 Reset All Votes", key="reset_votes_btn"):
...
def create_property_card(property_data, show_contact=False):
    """Create a styled property card with image and details"""
    ...
    with col2:
        ...
        st.caption(f"📍 {property_data.get('location', 'N/A')} | 🏷️ {property_data.get('type', 'N/A')}")
        ...
        with st.expander("🔍 Quick Facts"):
            ...
            - **Completion:** {property_data.get('completion', 'N/A')}
            - **Units Available:** {property_data.get('units', 'N/A')}
            - **Floors:** {property_data.get('floors', 'N/A')}
            """)
            
        if show_contact:
            st.info(f"📞 {property_data.get('contact', 'Contact sales for more info')}")
            
        if st.button("View Details", key=f"view_{property_data.get('name', '').lower().replace(' ', '_')}"):
...
def get_matching_properties(profile_type, properties_data):
    """Return properties matching the investor profile"""
    profile_mapping = {
        "Diaspora Investor": {
    ...
    return sorted(matched, key=filters['sort_key'])
...
# --- ALERTS PAGE ---
    ...
    if st.session_state.get("is_admin", False):
        ...
        # Display subscribers in an editable dataframe
        st.data_editor(
            subscribers_df,
            column_config={
                "email": "Email",
        ...
        if st.button("💾 Save Changes"):
            try:
                # Get edited data
                edited_data = st.session_state.subscribers_editor["edited_rows"]
                
                # Apply changes to the dataframe
                for idx, changes in edited_data.items():
                    for col, value in changes.items():
                        subscribers_df.at[idx, col] = value
